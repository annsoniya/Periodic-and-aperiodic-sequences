...
for q=1:4
    for jj=(q-1)*4+1:(q-1)*4+4
        pred_bins(jj,1:size(npred1,2))=npred1(q,:);% 40 ms before every period
        prev_bins(jj,1:size(nprev1,2))=nprev1(q,:);% from start of periodi till 40ms previous
    end
end
%% %
npers=[12 8 6 4];
...
%% Run this segemnt for three cases of stimuli combination
clc
