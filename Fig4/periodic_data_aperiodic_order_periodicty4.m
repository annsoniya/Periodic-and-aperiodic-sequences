...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data_aper_withmeanzero=data_aper_4;
...
% sort periodic data based on mean values from highest to lowest
sorted_data_per = data_per_4(sorted_indices_aper, :);
...
per4.AperInsig_perSig= intersect(zero_indices_aper, non_zero_indices_per);
% find cells sig in periodic ,but not sig in aper
per4.APerSig_perInSig = intersect(non_zero_indices_aper,zero_indices_per);
...
mean_diff_data_per=zeros(size(data_per_4,1),size(data_per_4{1,1},2));
diff_data_per=data_per_4;
...
%declare  mean_diff_data_aper;
mean_diff_data_aper=zeros(size(data_aper_4,1),size(data_aper_4{1,1},2));
diff_data_aper=data_aper_4;
