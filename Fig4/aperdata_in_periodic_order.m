...
sorted_indices_per=[];
sorted_values_per=[];[sorted_values_per, sorted_indices_per] = sort(value_per, 'descend');
% THIS HAS SORTED MEASN OF ALL SIGNIFICANT CELLS COMAPRED TO BASELINE . AND
% THE OTEHR MEAN VALUES WILL BE ZERO 

zero_indices_per = sorted_indices_per(sorted_values_per == 0);
% find non zero indices
non_zero_indices_per = sorted_indices_per(sorted_values_per ~= 0);

sorted_data_per = data_per(sorted_indices_per, :); % dataof all cells inthe sorted order 
...
%plot
sorted_data_aper = data_aper(sorted_indices_aper, :);
...
%% %%%%%%%%%%%%%%%%%segment 2%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% fr imagesc plots 
xxx=data_per(non_zero_indices_per,:);
for y = 1:size(xxx, 1)
    data_sig_cell_only_per(y, :) = mean(cat(1, xxx{y, :}));
end
data_sig_cell_only_per=sort(data_sig_cell_only_per, 'descend');
figure;
subplot(1, 2, 1)
imagesc(data_sig_cell_only_per(:, 9:33));
title('periodic sig cells(mean rates) ');

% aperiodic 
clear xxx;
xxx=data_per(non_zero_indices_aper,:);
for y = 1:size(xxx, 1)
    data_sig_cell_only_aper(y, :) = mean(cat(1, xxx{y, :}));
end
data_sig_cell_only_aper=sort(data_sig_cell_only_aper, 'descend');

subplot(1, 2, 2)
imagesc(data_sig_cell_only_aper(:, 9:33));
title('Aperiodic sig cells(mean rates) ');


%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%segment 3 - make the insignificant cells to zero mean data in periodic 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
data_per_withmeanzero=data_per;
...
figure
stem(mean(mean_sorted_data_per_withmeanzero(:, 9:33), 2),'r');
title('periodic data sig cells(mean rates) and periodic insig cells (meanrates=0) H_L');

%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%segment 4= change teh orderof aperiodic data according to periodic high to low)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% sort aperiodic data based on mean values from highest to lowest
sorted_data_aper = data_aper(sorted_indices_per, :);
for y = 1:size(sorted_data_aper, 1)
    mean_sorted_data_aper(y, :) = mean(cat(1, sorted_data_aper{y, :}));
end
...
figure;
subplot(1, 3, 1)
stem(mean(mean_sorted_data_aper(:, 9:33), 2),'Color', [1, 0.5, 0]);
title(' all APERIODIC DATA (1821) in the order of periodic indices H > L');
...
mean_diff_data_aper=zeros(size(data_aper,1),size(data_aper{1,1},2));
diff_data_aper=data_aper;
...
subplot(1, 3, 2)
stem(mean(mean_diff_data_aper(:, 9:33), 2),'Color', [1, 0.5, 0]);
xlabel('cells in teh order of periodic high to low');
title('Aperiodic data- zero mean for aperiodic insignificant cells , sorted(per - H > L)');
...
stem(mean(mean_diff_data_aper(:, 9:33), 2),'Color', [1, 0.5, 0]);
xlabel('cells in teh order of periodic high to low');
title('Aperiodic data (aper_insig= 0 + per_sig= 0) = exclusively aperiodic selctive ');
